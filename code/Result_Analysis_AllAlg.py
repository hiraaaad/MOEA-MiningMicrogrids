        ...
        for i, file in enumerate(file_list):
            ...
            if not Pareto_Flag:
                ...
                if not gen_column_added:
                    ...
                    df = pd.read_csv(file, usecols=["Gen (#)", "NPC ($/kWh)", "GHE (kg CO2/kWh)", "ENS (%)"])
                    gen_column_added = True  # Mark that the "Gen(#)" column is added
                    df.columns = ["Gen (#)",f"NPC_{i}", f"GHE_{i}", f"ENS_{i}"]
                else:
                    # Exclude the "Gen(#)" column for subsequent files
                    df = pd.read_csv(file, usecols=["NPC ($/kWh)", "GHE (kg CO2/kWh)", "ENS (%)"])
                
                # Rename the columns (except "Gen(#)") to distinguish between files
                if i > 0:
                    df.columns = [f"NPC_{i}", f"GHE_{i}", f"ENS_{i}"]
...
# Usage
names_X = ["NSGAII", "SMSEMOA", "UNSGA3", "AGEMOEA", "AGEMOEA2", "NSGA3"]
base_path = "Results//"
...
for X in names_X:
    ...
    for gen_value in range(genMax):
        ...
        for i in range(len(npc_cols)):
            ...
            mask = ens_i == 0
            gg = gen_data.iloc[mask][[npc_cols[i], ghe_cols[i]]]
            
            # Stack NPC and GHE values as a cost matrix
            costs = np.column_stack((npc_i[mask], ghe_i[mask]))  # Shape (n, 2)
            
            # Determine Pareto-efficient points
            pareto_mask = is_pareto_efficient(costs)
            
            # Retrieve the Pareto-optimal solutions for the current (Gen, i)
            pareto_solutions = gg.iloc[pareto_mask][[npc_cols[i], ghe_cols[i]]]
